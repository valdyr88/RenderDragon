...
//	specular functions
//--------------------------------------------------------------------------------------
float3 pbrSampleSpecular(in float dotNL, in float dotNV, in float dotNH, in float3 F, in float roughness, in float rim_lighting){
...
float3 pbrSampleSpecular(in float dotNL, in float dotNV, in float dotNH, in float roughness, in vec3 f0, in float rim_lighting){
...
#ifndef pbrSample_NofLights
	#define pbrSample_NofLights 4
#endif
...
float4 pbrSampleAmbient(const samplerCube txAmbient, float3 t, float roughness){
	return AmbientMult(AMBIENT_LIGHT_INCREASE_PERCENTAGE) * textureLod(txAmbient, t, AmbientMipMapLevels * roughness );
}
float4 pbrSampleAmbient(const samplerCube txAmbient, const float AmbientLightIncreasePercent, float3 t, float roughness){
	return AmbientMult(AmbientLightIncreasePercent) * textureLod(txAmbient, t, AmbientMipMapLevels * roughness );
}

#ifdef GAMMA_ENCODED_BRDF
float2 pbrSampleBRDF(const sampler2D txBRDF, float2 t){
	return gamma(texture(txBRDF, 0.999f*vec2(t.x,1.0-t.y)+0.001f ).xy, 1.0f/GAMMA_ENCODED_BRDF);
}
#else
float2 pbrSampleBRDF(const sampler2D txBRDF, float2 t){
	return texture(txBRDF, 0.999f*vec2(t.x,1.0-t.y)+0.001f ).xy;
}
#endif
//--------------------------------------------------------------------------------------
...
#define pbrSetupRoughness(x) (lerp(0.0f,0.85f,(x)))	//0.7f
...
float3 pbrSample(vec3 position, vec3 diffuse, vec3 normal, vec3 specular, float roughness, float metalness, const sampler2D txBRDF,
				  float shadows[pbrSample_NofLights], float ambientOcclusion, vec3 viewVector, const samplerCube AmbientTx,
				  const float AmbientLightIncreasePercent, SLight lights[pbrSample_NofLights], const int lightNo)
{
	...
	roughness = pbrSetupRoughness(roughness);
	...
	for(int i = 0; i < lightNo; ++i){
		...
		//	point light
		//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		if(getLightType(light.flags) == LightType_Point){
		...
		//  directional light
		//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		if(getLightType(light.flags) == LightType_Direction){
		...
		float3 spec = pbrSampleSpecular(dotNL, dotNV, dotNH, specfresnel, roughness, 0.0f) * dotNL;//moze i dotiNL
	...
	//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	float2 brdf = pbrSampleBRDF(txBRDF, float2(roughness, dotNV));
	float3 iblspec = (fresnel(specular, dotNV) * brdf.x + brdf.y );
	
	float3 ambdiff = ambientOcclusion * pbrSampleAmbient(AmbientTx, AmbientLightIncreasePercent, -N, pbrSetupRoughness(1.0f) ).xyz;//(ako cemo radit ikad vise ambijenata koji se blendaju onda treba lerpat ova dva parametra (ambdiff, i ambrefl))
	float3 ambrefl = lerp(1.0f, ambientOcclusion, roughness ) * pbrSampleAmbient(AmbientTx, AmbientLightIncreasePercent, R, roughness ).xyz;
...
void pbrSampleLight(vec3 position, vec3 diffuse, vec3 normal, vec3 specular, float roughness, float metalness,
					 float shadow, vec3 viewVector, SLight light,
					 out float3 lightdiff, out float3 lightrefl)
{
	...
	float3 L = vec3(0.0f); float dL2 = 1.0f;
	...
	float3 f = fresnel(specular, dotNV, roughness);
	float3 spec = pbrSampleSpecular(dotNL, dotNV, dotNH, f, roughness, 0.0f) * dotNL;//moze i dotiNL
	float3 diff = saturate((1.0f-f)*(1.0f-metalness) * dotNL);// * lambert_diffuse()
...
void pbrSampleAmbient(vec3 diffuse, vec3 normal, vec3 specular, float roughness, float metalness, const sampler2D txBRDF,
...
vec3 pbrIntegrateSamples(vec3 diffuse, float metalness, vec3 lightdiff, vec3 lightrefl){
	// lightdiff = lightdiff * lerp( diffuse.xyz, tofloat3(0.0f), metalness );
